...
(статус, исполнитель, заголовок, ссылка) в `structured.json → jira_issues`.
...
Подключение к Jira (хост, токен) настраивается в MCP-клиенте — в `team.yaml`
дублировать не нужно. Ссылку на задачу скилл берёт из ответа `jira_get_issue`.

Если в реплике явно назван другой префикс (`BILL-45`) — используется он.
Если `jira`-секции нет и префикс не обсуждали — скилл один раз спросит префикс.
