...
- Задачи Jira — обсуждавшиеся за период задачи со статусом и исполнителем
  (агрегируются из `jira_issues` в `structured.json`, последний снимок по каждой задаче)
...
- Исполняемость поручений по людям (требует актуальных статусов —
  см. сверку поручений в process-daily)
